...
// src/modules/users/useCases/createUser/CreateUserUseCase.spec.ts
var useCase;
var users;
var hash;
beforeEach(() => {
  ...
  useCase = new CreateUserUseCase(users, hash);
});
describe("CreateUserUseCase", () => {
  it("cria usu\xE1rio e hash da senha", async () => {
    const user = await useCase.execute({
      name: "John Doe",
      email: "john@example.com",
      password: "123456"
    });
    globalExpect(user.id).toBeDefined();
    globalExpect(user.password).toBe("hashed:123456");
    globalExpect(user.role).toBe("EMPLOYEE");
    globalExpect(user.active).toBe(true);
  });
  it("n\xE3o permite e-mail duplicado", async () => {
