...
ListBarbershopsUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe2.inject)("BarbershopRepository"))
...
UpdateBarbershopUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe4.inject)("BarbershopRepository"))
...
GetScheduleUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe6.inject)("BarbershopRepository"))
], GetScheduleUseCase);
...
// src/modules/barbershops/useCases/barbershops.spec.ts
var repo;
var create;
var list;
var get2;
...
beforeEach(() => {
  ...
  create = new CreateBarbershopUseCase(repo);
  list = new ListBarbershopsUseCase(repo);
  get2 = new GetBarbershopUseCase(repo);
  update = new UpdateBarbershopUseCase(repo);
  del = new DeleteBarbershopUseCase(repo);
  getSchedule = new GetScheduleUseCase(repo);
  updateSchedule = new UpdateScheduleUseCase(repo);
});
describe("Barbershops module", () => {
  it("cria, lista, busca e atualiza barbearia", async () => {
    ...
    globalExpect(items.length).toBe(1);
    const fetched = await get2.execute(b.id);
    globalExpect(fetched.name).toBe("Barber X");
    const updated = await update.execute(b.id, { name: "Barber Y" });
    globalExpect(updated.name).toBe("Barber Y");
  });
  it("deleta (desativa) barbearia", async () => {
    const b = await create.execute({ name: "B", whatsapp: "55", logoUrl: void 0 });
    await del.execute(b.id);
    const fetched = await repo.findById(b.id);
    globalExpect(fetched?.active).toBe(false);
  });
  it("agenda: atualiza e busca", async () => {
    ...
    globalExpect(sched.length).toBe(1);
    globalExpect(sched[0].dayOfWeek).toBe(1);
  });
  it("lan\xE7a erro ao buscar id inexistente", async () => {
    await globalExpect(get2.execute("not-found")).rejects.toBeInstanceOf(AppError);
  });
});
