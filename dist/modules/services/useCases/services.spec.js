...
ListServicesUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe2.inject)("ServiceRepository"))
], ListServicesUseCase);
...
UpdateServiceUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe4.inject)("ServiceRepository"))
...
// src/modules/services/useCases/services.spec.ts
var repo;
var create;
var list;
var get2;
...
beforeEach(() => {
  ...
  create = new CreateServiceUseCase(repo);
  list = new ListServicesUseCase(repo);
  get2 = new GetServiceUseCase(repo);
  update = new UpdateServiceUseCase(repo);
  del = new DeleteServiceUseCase(repo);
});
describe("Services module", () => {
  it("cria e lista servi\xE7os por barbearia", async () => {
    ...
    globalExpect(listAll.length).toBe(2);
    const listShop1 = await list.execute("shop-1");
    globalExpect(listShop1.length).toBe(1);
    globalExpect(listShop1[0].id).toBe(s1.id);
  });
  it("obt\xE9m servi\xE7o por id e atualiza", async () => {
    const s = await create.execute({ barbershopId: "shop-1", name: "Corte", price: 50, avgTimeMinutes: 30, icon: "scissors" });
    const fetched = await get2.execute(s.id);
    globalExpect(fetched.name).toBe("Corte");
    const updated = await update.execute(s.id, { price: 55 });
    globalExpect(updated.price).toBe(55);
  });
  it("desativa servi\xE7o", async () => {
    const s = await create.execute({ barbershopId: "shop-1", name: "Corte", price: 50, avgTimeMinutes: 30, icon: "scissors" });
