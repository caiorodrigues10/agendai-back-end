...
function printListItems(list2, config2, indentation, depth, refs, printer2) {
  let result = "";
  list2 = list2 instanceof ArrayBuffer ? new DataView(list2) : list2;
  const isDataView = (l) => l instanceof DataView;
  const length = isDataView(list2) ? list2.byteLength : list2.length;
  if (length > 0) {
    result += config2.spacingOuter;
    const indentationNext = indentation + config2.indent;
    for (let i = 0; i < length; i++) {
      ...
      if (isDataView(list2) || i in list2) {
        result += printer2(isDataView(list2) ? list2.getInt8(i) : list2[i], config2, indentationNext, depth, refs);
      }
      if (i < length - 1) {
        result += `,${config2.spacingInner}`;
      } else if (!config2.min) {
...
function inspectList2(list2, options, inspectItem, separator = ", ") {
  inspectItem = inspectItem || options.inspect;
  const size = list2.length;
  if (size === 0)
    return "";
  ...
  for (let i = 0; i < size; i += 1) {
    const last = i + 1 === list2.length;
    const secondToLast = i + 2 === list2.length;
    truncated = `${truncator2}(${list2.length - i})`;
    const value = list2[i];
    options.truncate = originalLength - output.length - (last ? 0 : separator.length);
    const string2 = peek || inspectItem(value, options) + (last ? "" : separator);
    const nextLength = output.length + string2.length;
    ...
    output += string2;
    if (!last && !secondToLast && nextLength + peek.length >= originalLength) {
      truncated = `${truncator2}(${list2.length - i - 1})`;
      break;
    }
    truncated = "";
  }
...
assert2.oneOf = function(inList, list2, msg) {
  new Assertion(inList, msg, assert2.oneOf, true).to.be.oneOf(list2);
};
...
ListQueueUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe2.inject)("QueueRepository"))
], ListQueueUseCase);
...
DeleteQueueItemUseCase = __decorateClass([
  ...
  __decorateParam(0, (0, import_tsyringe4.inject)("QueueRepository"))
], DeleteQueueItemUseCase);

// src/modules/queue/useCases/queue.spec.ts
init_AppError();
var queues;
var join2;
var list;
...
beforeEach(() => {
  ...
  join2 = new JoinQueueUseCase(queues);
  list = new ListQueueUseCase(queues);
  update = new UpdateQueueItemUseCase(queues);
  del = new DeleteQueueItemUseCase(queues);
});
describe("Queue module", () => {
  it("join queue e lista por barbearia", async () => {
    const q1 = await join2.execute({ barbershopId: "shop-1", customerName: "Jo\xE3o", whatsapp: "5599", serviceId: "svc-1", customerId: "cust-1" });
    const q2 = await join2.execute({ barbershopId: "shop-2", customerName: "Maria", whatsapp: "5598", serviceId: "svc-2", customerId: "cust-2" });
    const all = await list.execute();
    globalExpect(all.length).toBe(2);
    const onlyShop1 = await list.execute("shop-1");
    globalExpect(onlyShop1.length).toBe(1);
    globalExpect(onlyShop1[0].id).toBe(q1.id);
    globalExpect(q1.status).toBe("waiting");
  });
  it("atualiza status para in_chair e completed com pre\xE7o informado", async () => {
    const q = await join2.execute({ barbershopId: "shop-1", customerName: "Ana", whatsapp: "55", serviceId: "svc-1", customerId: "cust-1" });
    const inChair = await update.execute(q.id, "in_chair");
    globalExpect(inChair.status).toBe("in_chair");
    const completed = await update.execute(q.id, "completed", { completedBy: "staff-1", finalPrice: 50 });
    globalExpect(completed.status).toBe("completed");
    globalExpect(completed.finalPrice).toBe(50);
    globalExpect(completed.completedBy).toBe("staff-1");
    globalExpect(typeof completed.completedAt).toBe("number");
  });
  it("cancela item e remove do hist\xF3rico", async () => {
    const q = await join2.execute({ barbershopId: "shop-1", customerName: "Ana", whatsapp: "55", serviceId: "svc-1", customerId: "cust-1" });
    const cancelled = await update.execute(q.id, "cancelled");
    globalExpect(cancelled.status).toBe("cancelled");
    await del.execute(q.id);
    const listAll = await list.execute();
    globalExpect(listAll.find((i) => i.id === q.id)).toBeUndefined();
  });
  it("lan\xE7a erro ao atualizar item inexistente", async () => {
    await globalExpect(update.execute("not-found", "completed")).rejects.toBeInstanceOf(AppError);
  });
});
