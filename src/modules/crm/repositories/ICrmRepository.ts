...
export interface ICrmRepository {
  ...
  listClients(barbershopId: string, params: { page: number; limit: number; search?: string; segment?: CrmSegment; sort?: "ltv" | "lastVisit" | "outstanding" }): Promise<{ data: CrmClientMetrics[]; total: number }>;
  getClientProfile(barbershopId: string, clientId: string): Promise<Record<string, unknown> | null>;
