...
export async function barbershopsRoutes(app: FastifyInstance) {
  ...
  app.put("/barbershops/:id", { preHandler: [authenticate, authorize(["MASTER_ADMIN", "OWNER"])] }, update.handle.bind(update));
  ...
  app.put("/barbershops/:id/schedule", { preHandler: [authenticate, authorize(["MASTER_ADMIN", "OWNER"])] }, updateSchedule.handle.bind(updateSchedule));
}
